...
export const PORTAL_SYNC_KEYS = [
  ...
  "xeva_selticket_v1",
  /* ★★ 2026-09-13 フェスセレクト券（フェスごとに「好きな1体」をえらべる券）。
     中身はフェスのキーごとの { e:もらった総数, u:使った総数 }。
     買ってすぐ使うので xeva-cloud.js の URGENT_KEYS にも入れてある。 */
  "xeva_fessel_v1",
  /* ★★ 2026-09-13c ★星煌印（ガチャの天井）。fesSelect と同じ {e,u} 形なので
     COUNT_KEYS（項目ごとに max で混ぜる）に入れること。 */
  "xeva_seal_v1",
  /* ★★ 2026-08-30 💠結晶（プレミアムセレクトのキャラが完凸後に出たときにもらえる）。
     75個で PREMIUM SELECT / GRAND DEBUT の好きな1体と交換できるので、
     残高がずれると「交換したのに別の端末ではまだ持っている」が起きる。必ず同期する。 */
  "xeva_cryst_v1",
  ...
  "xevynar_v1",
  /* ★★ 2026-08-19 XEVYNAR が<b>会話から覚えたこと</b>と<b>解説をどこまで見たか</b>。
     ここを同期しないと、端末を変えたとたんに
       ・同じ聞きかたがまた通らなくなる（覚えた言いかたが消える）
       ・どの解きかたを読み終えたかが分からなくなる
     ＝「自分のことを知っているAI」でなくなってしまう。
...
export const APP_SYNC_KEYS = {
  ...
  magilex: [
    ...
    "magilex_arisa_v1",
    "magilex_flash_v1",
    /* ★★ 2026-08-19 メモとフラッシュカードも同期する。
       手で書いた計算・自分で作ったカードは<b>作り直しがきかない</b>ので、
       端末を変えたら消えている、が起きてはいけない。 */
    "magilex_memo_v1",      // メモ（手書きの線・文字）
    "magilex_cards_v1",     // 自分で作ったフラッシュカード
    "magilex_memo_full_v1", // メモを広げていたか
    "magilex_memo_autoclear_v1",
  ],
};
