...
(function () {
  ...
  var DEF = {
    ...
    /* ★★ 2026-08-29 同期のしかたの警告（ご指定・既定はオン＝毎回出す）。
       iPhone / iPad では、アプリの画面をそのまま閉じると同期が届かないことがある。
       「終わるときは各アプリの左上のホームボタンでホームへ戻る」を<b>起動のたびに</b>知らせる。
       ★ この設定は<b>端末ごと</b>（localStorage）。同期されているPC・Android で切っても、
         iPhone 側の警告は消えない——それでよい（困っているのは iPhone だけなので）。 */
    syncWarn: 1          /* 1=起動のたびに同期の注意を出す 0=出さない */
  };
