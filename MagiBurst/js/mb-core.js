...
/* ══════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════ */
const GACHA_VER = "1.0";                 // ★ キャラを入れ替えるたびに上げる
const GACHA_VER_DATE = "2026-08-20";     // その版の公開日
function gachaVerText() { return "Ver." + GACHA_VER; }
...
/* ★ いまの GRAND DEBUT の顔ぶれ（＝この版で追加された新キャラ）。
   ★★ DEBUT_MIGRATE_HOWTO ── 次の更新でキャラを入れ替えるときの手順
     ① いまの DEBUT_CHARS の中身を <b>PREMIUM_CHARS の末尾</b>へ移す
        （移した時点で、プレミアムのピックアップ・すり抜け・確定枠・フェスのすり抜けに
          そのまま乗る＝「現行キャラと同じ扱い」になる。ほかに直す場所はない）
     ② DEBUT_CHARS を新しいキャラだけにする
     ③ GACHA_VER を上げ、GACHA_VER_DATE を更新する
     ④ mb-newchars.js の where / mode（"debut"）はそのままでよい（新しいキャラぶんを足す） */
const DEBUT_CHARS = ["mirelle", "scarlet", "koyuki", "amelia", "mio"];
function isDebutMode(m) { return m === "debut"; }
/* 排出対象（限界突破MAXは除外し、そのぶんを残りで分け合う＝フェスと同じ考えかた） */
function debutPool() { return DEBUT_CHARS.filter((id) => !isMaxAwk(id)); }
function debutEachRate() { const n = debutPool().length; return n ? S5_TOTAL / n : 0; }
...
const GACHA_CHARS = PREMIUM_CHARS.concat(FES_ALL_CHARS, DEBUT_CHARS);   // 図鑑・互換用（全ガチャの排出キャラ）
...
function newestGachaMode() {
  ...
  try {
    ...
    /* ★ 2026-08-20 いちばん新しいキャラは必ず GRAND DEBUT にいるので、既定はここになる */
    DEBUT_CHARS.forEach((id) => consider(id, "debut"));
...
function gachaMenuList() {
  const list = [
    ...
    { k: "debut", nm: DEBUT_NM, sub: gachaVerText() + "・新キャラ" + DEBUT_CHARS.length + "体はここだけ", c: DEBUT_C, ver: GACHA_VER },
...
/* ══════════ ★ 2026-08-20 GRAND DEBUT GACHA の抽選 ══════════
   ...
   ・SSR 合計10%   … <b>GRAND DEBUT のキャラ</b>で等分（ピックアップなし）
   ・SSR 合計5%    … <b>プレミアムのSSR</b>も「すり抜け」で出る（FES_PREMIUM_TOTAL）
   ・SR  合計50%   … STAR4_POOL で等分（フェスと同じ50%。プレミアムは55%）
   ・残り35%       … D_ITEM_TABLE（GRAND DEBUT 専用の中身）
   ★ <b>逆向きの約束は変わらない</b>——GRAND DEBUT のキャラは DEBUT_CHARS にしか居ないので、
     フェスやプレミアムの抽選（gachaPool＝PREMIUM_CHARS を見る）には今までどおり出ない。 */
function debutRollOnce() {
  const pool = debutPool();
  let r = Math.random();
  const each = debutEachRate();
  for (const o of pool) { if (r < each) return grantChar(o); r -= each; }        // 新キャラSSR（等分・合計10%）
  ...
  for (const s of STAR4_POOL) { if (r < s4) return grantChar(s); r -= s4; }
  return rollGachaItem(Math.random() * 0.35, D_ITEM_TABLE);                     // 残り35%＝GRAND DEBUT の中身
}
/* 10連の確定枠の母集団。フェス（fesSurePool）と同じで、
   <b>GRAND DEBUT のキャラ ＋ プレミアムのSSR</b> をまとめて全員おなじ確率にする。 */
function debutSurePool() {
  const seen = new Set(), out = [];
  debutPool().concat(gachaPool()).forEach((id) => {
    if (seen.has(id)) return;
    seen.add(id); out.push(id);
  });
  return out;
}
function debutGuaranteedS5() {
  const pool = debutSurePool();
  if (!pool.length) return debutRollOnce();      // 全員が限界突破MAX＝確定枠も通常抽選に戻す
  return grantChar(pool[Math.floor(Math.random() * pool.length)]);
}
function doDebutGacha(n) {
  n = n === 10 ? 10 : n === 5 ? 5 : 1;
