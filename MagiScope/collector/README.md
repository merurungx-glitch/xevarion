...
## ★ register-task.bat が動かないとき（2026-09-21e に直しました）
- 前の版は、bat の中の日本語と改行の形のせいで Windows が行を読みちがえ、途中で止まっていました。
  いまの bat は英数字だけにして、日本語の案内は register-task.ps1 に移しています。
- 「サインインしたとき」の引き金を**全ユーザー向け**に作っていたため、管理者でないと
  Access is denied になっていました。いまは**自分のユーザーだけ**に作るので、管理者はいりません。
- 状態を見たいときは **check-task.bat**（登録のようす・前回と次回の時刻・最後の取得の記録）。
- 取得の記録は `%LOCALAPPDATA%\MagiScope\last-run.log` に残ります。
