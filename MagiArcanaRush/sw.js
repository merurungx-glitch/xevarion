/* ============================================================
   Magi: Arcana Rush Service Worker

   ・画面・盤面・演出はすべて端末の中で動くので、一式を先に持っておけば
     <b>オフラインでもクエストが遊べる</b>。
   ・キャラクターの絵（../img/）は<b>数が多い</b>ので先読みしない——
     一度見たものが fetch のときに自然にたまる（MagiBurst 側の SW も持っている）。
   ・作りは Magi Lotto / MagiJackpot の SW と同じ
     （自分の接頭辞のキャッシュだけ掃除する／xev-refresh で差分更新できる）。
   ============================================================ */
const VERSION = "magiarcanarush-sw-v2";
const CORE = [
  "./",
  "./index.html",
...
self.addEventListener("install", (e) => {
  e.waitUntil((async () => {
    ...
    let done = 0;
    await post({ type: "xev-precache", scope: "magiarcanarush", done: 0, total: CORE.length });
    for (const u of CORE) {
      try { await cache.add(u); } catch (err) { try { await cache.add(u); } catch (e2) {} }
      done++;
      await post({ type: "xev-precache", scope: "magiarcanarush", done, total: CORE.length });
    }
    self.skipWaiting();
  })());
});
...
self.addEventListener("activate", (e) => {
  e.waitUntil((async () => {
    ...
    await Promise.all(keys.filter((k) => k !== VERSION && k.startsWith("magiarcanarush-sw")).map((k) => caches.delete(k)));
...
const XEV_SCOPE = "magiarcanarush";
