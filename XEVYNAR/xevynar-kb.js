...
(function () {
  ...
  /* ════════════ キャラ一覧 ════════════
     [id, 名前, 属性, 撃種, 戦型, SSRか, アビリティ, サブリンク, フルバースト名, リンクスキル名] */
  const CHAR_ROWS = [
  ...
  /* 入手先の説明 */
  const POOL_NM = {
    premium: ["プレミアムセレクトガチャ", "ジェムで回すメインのガチャ。10連は最後の1枠がSSR確定。ピックアップを自分で選べる。"],
    fes: ["Nocturne Bloom Fest", "フェス限定SSR。プレミアムからは出ない。消費は🎫フェスチケット（フェス専用）→🎫ガチャチケット（全ガチャ共通・2026-08-13 新設）→💎ジェムの順。"],
  ...
  const APPS = [
    ...
    { id: "magilotto", nm: "Magi Lotto", cat: "ゲーム", href: "../MagiLotto/index.html",
      sum: "💎ジェムで買って XEVA で受け取るデジタル宝くじ。中身は5本柱だけ。",
      det: "<b>SCRATCH</b>（9マスを削って同じ絵柄3つ／×1〜×1,000）、"
        + "<b>NUMBERS</b>（0〜9を3つ・ストレートは×500・MAGI PICK で当たりやすさは変わらない）、"
        + "<b>LOTTO</b>（1〜36から6個・5個一致×1,000／6個一致×20,000）、"
        + "<b>MAGI GRAND DRAW</b>（毎月1日と16日だけの大型抽選。1等は賞金プール全額で最低 1,000,000 XEVA を保証）、"
        + "<b>FREE MAGI</b>（1日1回 無料のルーレット）の5つ。"
        + "全体の期待還元率はおよそ<b>92%</b>で、確率と配当はゲーム内の「ガイド」にすべて出ている。"
        + "<b>人数が集まるのを待つ仕組みはなく、ひとりでも全機能が成立する</b>（賞金を取り合うこともない）。"
        + "抽選はサーバー側で決まり、買った時点で結果が確定して台帳に残るので、通信が切れても報酬は失われない。"
        + "オフラインでも遊べる（そのときは「ローカル抽選」の印が付く）。" },
  ...
  const FAQ = [
    ...
    { id: "gem", q: ["ジェム", "じぇむ", "gem", "宝石", "ダイヤ", "オーブ"],
      ...
      a: "<b>ジェム</b>は XEVARION 共通のプレミアム通貨です（もとは MagiBurst 専用でした）。<br>" +
        ...
        "・となりの<b>🛒パックストア</b>には<b>買い切りのパック</b>があり、通常より 50〜70% 多く受け取れます<br>" +
    ...
    { id: "gacha", q: ["ガチャ", "召喚", "確定", "天井", "排出", "10連", "ピックアップ"],
      ...
      a: "XEVARION 本体の<b>XEVAガチャ</b>と、MagiBurst の<b>プレミアムセレクトガチャ</b>（ジェム）があります。<br>" +
        "MagiBurst は 1回💎5／5連💎25／<b>10連💎50（最後の1枠がSSR確定）</b>。ピックアップは自分で選べます。<br>" +
        "フェス限定SSRはプレミアムからは出ません。<b>Nocturne Bloom Fest</b> で、🎫ガチャチケット（どのガチャでも使える共通チケット）があれば優先して消費されます。" },
  ...
  function poolsOf(charId) {
    ...
    /* 上のどれでもないSSRは、プレミアムセレクトガチャの排出とみなす
       （初期4体などのSSRでないキャラは最初から持っているので入手先を出さない） */
    const c = CHAR_BY_ID[charId];
