/* ============================================================
   Magi Dominion Grid Service Worker — オフライン対応
   ・1台で遊ぶボードゲームなので、キャッシュしてしまえば完全にオフラインで動く
   ・記録（mdg_records_v1）は localStorage
   ・Firebase など外部通信はキャッシュしない
   ============================================================ */
const VERSION = "mdgrid-sw-v2";
const CORE = [
  "./index.html",
  "./css/mdg.css?v=4",
  "./js/mdg.js?v=3",
...
self.addEventListener("install", (e) => {
  e.waitUntil((async () => {
    ...
    await xevPrecache(cache, CORE, "magidominiongrid");
    self.skipWaiting();
  })());
});
...
self.addEventListener("activate", (e) => {
  e.waitUntil((async () => {
    ...
    await Promise.all(keys.filter((k) => k !== VERSION && k.startsWith("mdgrid-sw")).map((k) => caches.delete(k)));
...
/* ══════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════ */
const XEV_SCOPE = "magidominiongrid";
